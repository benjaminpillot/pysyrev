...
def _build_bib_network_section(bib_network_config, section_n,
    ...
    graphs = [
        (bib_network_config.coupling_graph,   "Bibliographic coupling", None,                  "Title"),
        (bib_network_config.cocitation_graph, "Co-citation",            _COCITATION_MAX_NODES, "Reference"),
    ]

    # Build a base topic map {paper_id: topic_int} from bertopic_results
    _id_topic_base: dict = {}
    ...
    sub_blocks = []
    for path, label, max_nodes, node_col_header in graphs:
        ...
        is_cocitation = max_nodes is not None
        ...
        sub_content = [
            {
                ...
                "items": [
                    ...
                    {"key": "Average degree", "value": f"{avg_deg:.2f}"},
                ],
            }
        ]
        if node_rows:
            if is_cocitation:
                headers    = [node_col_header, "Degree", "DOI", "In corpus"]
                col_widths = [8.5, 1.5, 4.5, 2.5]
            else:
                headers    = [node_col_header, "Degree", "DOI"]
